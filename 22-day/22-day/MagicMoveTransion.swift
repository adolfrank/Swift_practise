...
class MagicMoveTransion: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(transitionContext: UIViewControllerContextTransitioning) {
        ...
        let snapshotView = fromVC.selectedCell.cardImage.snapshotViewAfterScreenUpdates(false)
        snapshotView.frame = container!.convertRect(fromVC.selectedCell.cardImage.frame, fromView: fromVC.selectedCell)
        fromVC.selectedCell.cardImage.hidden = true
        ...
        */
        toVC.avatarImage.layoutIfNeeded()
        UIView.animateWithDuration(transitionDuration(transitionContext), delay: 0, options: UIViewAnimationOptions.CurveEaseInOut, animations: { () -> Void in
                snapshotView.frame = toVC.avatarImage.frame
                toVC.view.alpha = 1
            }) { (finish: Bool) -> Void in
                fromVC.selectedCell.cardImage.hidden = false
                toVC.avatarImage.image = toVC.image
